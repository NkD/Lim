...
public abstract class PositionalLight extends Light {
    ...
    void setMesh() {
        if (rayHandler.isGL20) {
            ...
            segments[size++] = start.x * body2WorldScale;
            segments[size++] = start.y * body2WorldScale;
            segments[size++] = colorF;
            segments[size++] = 1;
            // rays ending points.
            for (int i = 0; i < rayNum; i++) {
                segments[size++] = mx[i] * body2WorldScale;
                segments[size++] = my[i] * body2WorldScale;
                segments[size++] = colorF;
                segments[size++] = 1 - f[i];
            }
            ...
            for (int i = 0; i < rayNum; i++) {
                ...
                final float s = (1 - f[i]);
                segments[size++] = s;
                segments[size++] = mx[i] * body2WorldScale + s * softShadowLenght * cos[i];
                segments[size++] = my[i] * body2WorldScale + s * softShadowLenght * sin[i];
                segments[size++] = zero;
                segments[size++] = 0f;
            }
            ...
            for (int i = 0; i < rayNum; i++) {
                ...
                // ugly inlining
                segments[size++] = Float.intBitsToFloat(((int) (a * s) << 24) | ((int) (b * s) << 16) | ((int) (g * s) << 8) | ((int) (r * s)) & 0xfeffffff);
            }
            lightMesh.setVertices(segments, 0, size);
    ...
    PositionalLight(RayHandler rayHandler, int rays, Color color, float distance, float x, float y, float directionDegree, float body2WorldScale) {
        super(rayHandler, rays, color, directionDegree, distance, body2WorldScale);
        start.x = x;
        start.y = y;
        sin = new float[rays];
        cos = new float[rays];
        ...
        if (rayHandler.isGL20) {
            lightMesh = new Mesh(staticLight, vertexNum, 0, new VertexAttribute(Usage.Position, 2, "vertex_positions"), new VertexAttribute(Usage.ColorPacked, 4, "quad_colors"), new VertexAttribute(Usage.Generic, 1, "s"));
            softShadowMesh = new Mesh(staticLight, vertexNum * 2, 0, new VertexAttribute(Usage.Position, 2, "vertex_positions"), new VertexAttribute(Usage.ColorPacked, 4, "quad_colors"), new VertexAttribute(Usage.Generic, 1, "s"));
