...
import cz.nkd.lim.ExampleLibGdx;
...
public abstract class PositionalLight extends Light {
	...
	void setMesh() {
		if (rayHandler.isGL20) {
			...
			segments[size++] = start.x * ExampleLibGdx.B2W;
			segments[size++] = start.y* ExampleLibGdx.B2W;
			segments[size++] = colorF;
			segments[size++] = 1;
			// rays ending points.
			for (int i = 0; i < rayNum; i++) {
				segments[size++] = mx[i]* ExampleLibGdx.B2W;
				segments[size++] = my[i]* ExampleLibGdx.B2W;
				segments[size++] = colorF;
				segments[size++] = 1 - f[i];
			}
			...
			for (int i = 0; i < rayNum; i++) {
				...
				final float s = (1 - f[i]);
				segments[size++] = s;
				segments[size++] = mx[i]* ExampleLibGdx.B2W + s * softShadowLenght * cos[i];
				segments[size++] = my[i]* ExampleLibGdx.B2W + s * softShadowLenght * sin[i];
				segments[size++] = zero;
				segments[size++] = 0f;
			}
